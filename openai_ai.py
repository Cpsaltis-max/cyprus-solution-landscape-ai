...
def extract_sources(response: Any) -> list[dict[str, Any]]:
    """Return visible, benchmark-ready sources from File Search results."""
    ...
    for item in data.get("output", []) or []:
        ...
        if item.get("type") == "file_search_call":
            for result in item.get("results", []) or []:
                ...
                citation = filename
                if page is not None:
                    citation = f"{filename}, PDF p. {page}"
                sources.append(
                    {
                        "citation": citation,
                        "title": filename,
    ...
    # File citations remain useful even if detailed search results are unavailable.
    for item in data.get("output", []) or []:
        ...
        for content in item.get("content", []) or []:
            ...
            for annotation in content.get("annotations", []) or []:
                ...
                key = (file_id or filename, None)
                if any((s.get("file_id") or s["title"]) == key[0] for s in sources):
                    continue
                sources.append(
                    {
                        "citation": filename,
...
def ask_openai(
    *,
    ...
    """Answer using dashboard data, OpenAI File Search, or both."""
    ...
    if mode == "data_only":
        ...
        source_rule = (
            "Use only passages retrieved from the indexed 45 papers and Conflict and Change. "
            "Do not use the dashboard tables or outside knowledge."
        )
    else:
        source_rule = (
            "Use the supplied dashboard tables for empirical findings and retrieved papers/book "
            "for interpretation. Clearly separate those two kinds of evidence."
        )

    instructions = """
You are the research assistant for the Cyprus Solution Landscape Dashboard.
Use only the evidence allowed for the selected mode. Never invent a study, title, year,
percentage, sample, causal claim, quotation, or citation. If the evidence is insufficient,
say so explicitly. Distinguish 'in favor' from 'accepted': accepted = in_favor + tolerate.
Joint acceptance is the lower of GC and TC acceptance. When using dashboard data, state
the year, community, solution, and exact percentage. For every scholarly claim, name the
retrieved paper or book and give the PDF page when a page marker is available. Distinguish
general theory or meta-analysis from evidence specifically collected in Cyprus. Answer in
the language of the question. Be concise but analytically useful.
""".strip()
...
DASHBOARD DATA:
{data_context or 'No dashboard data supplied in this mode.'}
